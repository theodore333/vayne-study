...
"""`,
    ...
    connect: `=== РЕЖИМ: СВЪРЖИ ===
Предмет: ${subjectName}

Ще даваш ДВЕ концепции от предмета и студентът трябва да обясни ВРЪЗКАТА.
Твоята задача:
1. Избери две свързани концепции от темите по-долу
2. Кажи: "Каква е връзката между [X] и [Y]?"
3. Ако студентът не може - задай насочващ въпрос (НЕ казвай връзката!)
4. Ако отговорът е частичен - питай за пропуснатите аспекти
5. След успешен отговор - дай нова двойка концепции
  ...
  try {
    ...
    if (messages.length === 0) {
      const startPrompts: Record<string, string> = {
        ...
        connect: 'Дай ми две концепции да свържа.',
